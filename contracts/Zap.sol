...
contract Zap is ERC721, EIP712, Ownable {
    ...
    function _mint(SeriesPublication memory publication, bytes4 serialNumber) private {
        // Implied by the requirement that serialNumber < publication.seriesTotal. seriesTotal must be >=0.
        // require(publication.seriesTotal > 0, "Zap: seriesTotal must be greater than 0.");
        ...
        if(existingZapPublication.seriesTotal == 0) {
            ...
            require(existingZapPublication.seriesTotal == publication.seriesTotal, "Zap: Different seriesTotal than previously published");
        }

        require(serialNumber < publication.seriesTotal, "Zap: Mint of a serialNumber greater than or equal to the seriesTotal.");
        ...
        returns (string memory)
    {
        ...
        return _baseURI(); // + Base58Encode(_tokenIPFSHashes[tokenId])
    }
}
